...
for each max-size value, a separate CSV file is written whose name is suffixed
with the max-size (e.g. image_evaluation_musiq_results_512.csv).

Usage examples:

  # Default, evaluate at max-size 512 only
    ...
    for max_size in max_sizes:
        ...
        with open(csv_path, "w", newline="", encoding="utf-8") as f:
            ...
            for i, img_path in enumerate(images, start=1):
                ...
                musiq_out = evaluate_musiq_tf(img_path, max_size=max_size if max_size > 0 else None)
        ...
        print(f"Done for max-size={max_size}. Results saved to {csv_path}")


def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate images with MUSIQ only; save CSV results for one or more "
            "resize max-size values (separate CSV per size)."
        )
    )
    parser.add_argument(
        "directory",
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    run_musiq_for_sizes(root, images, max_sizes, args.output_prefix)
    return 0
